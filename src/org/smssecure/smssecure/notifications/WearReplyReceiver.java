...
/**
 * Get the response text from the Wearable Device and sends an SMS as a reply
 *
 * @author  Alix Ducros
 */
public class WearReplyReceiver extends BroadcastReceiver
{
    ...
    public void onReceive(final Context context, Intent intent)
    {
        ...
        final long         recipientId  = intent.getLongExtra("recipientId", -1);
        final CharSequence responseText = remoteInput.getCharSequence(MessageNotifier.EXTRA_VOICE_REPLY);


        final Recipient recipient = new RecipientProvider().getRecipient(context, recipientId, false);

        if (threadIds != null && masterSecret != null) {
            Log.w("WearReplyReceiver", "threadIds length: " + threadIds.length);
            ...
            new AsyncTask<Void, Void, Void>() {
                ...
                protected Void doInBackground(Void... params) {
                    ...
                    MessageSender.send(context, masterSecret, reply, -1, true);
