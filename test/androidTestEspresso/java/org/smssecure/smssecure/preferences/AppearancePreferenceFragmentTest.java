...
@LargeTest
public class AppearancePreferenceFragmentTest extends SMSSecureEspressoTestCase<ConversationListActivity> {
  ...
  private void checkViewsMatchPreferences() throws Exception {
    // todo :|
    final String theme = SMSSecurePreferences.getTheme(getContext());
    onData(Matchers.<Object>allOf(withKey("pref_theme"), withSummaryText(theme)))
          .check(matches(isDisplayed()));

    final String language = SMSSecurePreferences.getTheme(getContext());
